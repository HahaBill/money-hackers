...
[![FastAPI](https://img.shields.io/badge/API-FastAPI-356AE6?style=for-the-badge&labelColor=242424&logo=fastapi&logoColor=356AE6)](https://fastapi.tiangolo.com)
...
https://github.com/user-attachments/assets/d33aa799-c2a0-4eab-827d-330ac4c1d6c7
...
- Traces every model, tool, and graph entry point in PRISM so a run is observable — unwired code is invisible.
...
flowchart LR
  ...
  rcg --> prism[PRISM traces]
  workbook --> owner([Owner])
  chat --> owner
  voice --> owner
  prism --> observe([Observe → Improve → Prove])
```

The deciding is done by five parts:
...
| Surfaces | SvelteKit workbook + audit trail; FastAPI `/chat` grounded in the run; ElevenLabs voice that only sequences validated text | Built |
| PRISM | Traces analysis runs, LLM calls, Tavily, and voice tools (`prismtrace-sdk`) — Observe → Improve → Prove | Built |
...
ELEVENLABS_API_KEY=
ELEVENLABS_AGENT_ID=             # optional; full Agents Platform session
ELEVENLABS_VOICE_ID=             # optional; dock TTS (default George)
...
| --- | --- | --- |
| Engine | Python, DuckDB | Deterministic arithmetic, Shapley, the reasoning & calculation graph |
| API | FastAPI, Uvicorn | `/runs`, `/dashboard`, `/chat`, `/voice/*`, ElevenLabs server tools |
| UI | SvelteKit, Vite, TypeScript | Desktop workbook; `/api` proxied to FastAPI |
| Reasoning | OpenAI (`gpt-6-astra`) | Classification, judgment, and chat — never arithmetic |
